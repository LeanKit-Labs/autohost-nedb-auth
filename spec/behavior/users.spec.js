...
var users = require( '../../src/nedb/users.js' );
...
describe( 'Users', function() {
	describe( 'when creating', function() {
		...
		describe( 'when changing password', function() {
			...
			before( function( done ) {
				seq( [ function() {
						return users.changePassword( 'userone', 'four', 'five' );
					}, function() {
						return users.getByName( 'userone' );
					}
				] )
					.then( null, function( err ) {
						console.log( err.stack );
		...
		describe( 'when creating tokens', function() {
			...
			before( function( done ) {
				seq( [ function() {
						return users.createToken( 'userone', 'six' );
					}, function() {
						return users.createToken( 'userone', 'seven' );
					}, function() {
						return users.createToken( 'userone', 'eight' );
					}, function() {
						return users.getTokens( 'userone' );
					}, function() {
						return users.getByName( 'userone' );
					}
				] )
		...
		describe( 'when destroying token', function() {
			...
			before( function( done ) {
				seq( [ function() {
						return users.destroyToken( 'userone', 'seven' );
					}, function() {
						return users.getByName( 'userone' );
					}
				] )
		...
		describe( 'when changing user roles', function() {
			...
			before( function( done ) {
				seq( [ function() {
						return users.changeRoles( 'userone', [ 'r1', 'r2', 'r3', 'r4' ], 'add' );
					}, function() {
						return users.changeRoles( 'userone', [ 'r6', 'r2', 'r4', 'r5' ], 'add' );
					}, function() {
						return users.changeRoles( 'userone', [ 'r3', 'r1', 'r5' ], 'remove' );
					}, function() {
						return users.getRoles( { name: 'userone' } );
					}
				] )
					.then( null, function( err ) {
						console.log( err );
		...
		describe( 'when checking and no users', function() {
			...
			it( 'should report no users', function( done ) {
				users.hasUsers()
					.then( function( x ) {
						x.should.be.false; // jshint ignore:line
						done();
					} );
			} );
		} );
